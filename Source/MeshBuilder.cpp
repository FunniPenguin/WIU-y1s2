...
Mesh* MeshBuilder::GenerateQuad(const std::string& meshName, glm::vec3 color, float length)
{
	...
	v.pos = glm::vec3(0.5f * length, -0.5f * length, 0.f); v.normal = glm::vec3(0, 0, 1); v.texCord = glm::vec2(1, 0);
	vertex_buffer_data.push_back(v); //v0
	v.pos = glm::vec3(0.5f * length, 0.5f * length, 0.f); v.normal = glm::vec3(0, 0, 1); v.texCord = glm::vec2(1, 1);
	vertex_buffer_data.push_back(v); //v1
	v.pos = glm::vec3(-0.5f * length, 0.5f * length, 0.f); v.normal = glm::vec3(0, 0, 1); v.texCord = glm::vec2(0, 1);
	vertex_buffer_data.push_back(v); //v2
	v.pos = glm::vec3(-0.5f * length, -0.5f * length, 0.f); v.normal = glm::vec3(0, 0, 1); v.texCord = glm::vec2(0, 0);
...
Mesh* MeshBuilder::GenerateText(const std::string& meshName, unsigned numRow, unsigned numCol)
{
	...
	for (unsigned row = 0; row < numRow; ++row)
	{
		for (unsigned col = 0; col < numCol; ++col)
		{
			...
			v.texCord = glm::vec2(width * (col + 1), height * (numRow - row));
			vertex_buffer_data.push_back(v);
			v.pos = glm::vec3(-0.5f, 0.5f, 0.f);
			v.normal = glm::vec3(0, 0, 1);
			v.texCord = glm::vec2(width * (col + 0), height * (numRow - row));
			vertex_buffer_data.push_back(v);
			v.pos = glm::vec3(-0.5f, -0.5f, 0.f);
			v.normal = glm::vec3(0, 0, 1);
			v.texCord = glm::vec2(width * (col + 0), height * (numRow - 1 - row));
			vertex_buffer_data.push_back(v);
			v.pos = glm::vec3(0.5f, -0.5f, 0.f);
			v.normal = glm::vec3(0, 0, 1);
			v.texCord = glm::vec2(width * (col + 1), height * (numRow - 1 - row));
			vertex_buffer_data.push_back(v);
	...
	glm::vec3 color, float radius, int numSlice, int numStack)
{
	...
	for (unsigned stack = 0; stack < numStack + 1; ++stack) //stack
	{
		...
		for (unsigned slice = 0; slice < numSlice + 1; ++slice) //slice
		{
			...
			v.normal = glm::normalize(glm::vec3(glm::cos(phi) * glm::cos(theta),
				glm::sin(phi),
...
Mesh* MeshBuilder::GenerateRing(const std::string& meshName, glm::vec3 color, unsigned numSlice, float innerR, float outerR)
{
	Vertex v;
	v.color = color;
	std::vector<Vertex> vertex_buffer_data;
...
Mesh* MeshBuilder::GenerateHemisphere(const std::string& meshName, glm::vec3 color, unsigned numStack, unsigned numSlice, float radius)
{
	...
	for (unsigned stack = 0; stack < numStack + 1; ++stack) //stack //replace with 180 for sphere
	{
		...
		for (unsigned slice = 0; slice < numSlice + 1; ++slice) //slice
		{
			...
			v.normal = normalize(glm::vec3(cosf(phi) * cosf(theta), sinf(phi), cosf(phi) * sinf(theta)));
			vertex_buffer_data.push_back(v);
		}
	}
	for (unsigned stack = 0; stack < numStack; ++stack)
	...
	for (unsigned stack = 0; stack < numStack + 1; ++stack) //stack
	{
		...
		for (unsigned slice = 0; slice < numSlice + 1; ++slice) //slice
		{
			...
			v.pos = glm::vec3((outerR + innerR * glm::cos(theta)) * glm::
				...
				glm::cos(phi));
			v.normal = glm::normalize(glm::vec3((innerR * glm::cos(theta)) * glm::sin(phi),
				innerR * glm::sin(theta),
				(innerR * glm::cos(theta)) * glm::cos(phi)));
			vertex_buffer_data.push_back(v);
		}
	}
	for (unsigned stack = 0; stack < numStack; ++stack)
...
Mesh* MeshBuilder::GenerateHollowCylinder(const std::string& meshName, glm::vec3 color, unsigned numSlice, float radius1, float radius2, float height)
{
	Vertex v;
	v.color = color;
	std::vector<Vertex> vertex_buffer_data;
	...
	{
		float theta = slice * degreePerSlice;
		v.pos = glm::vec3(radius1 * cos(theta), -0.5f * height, radius1 * sin(theta));
		v.normal = glm::vec3(0, -1, 0);
		vertex_buffer_data.push_back(v);
	}
	...
	for (int slice = 0; slice <= numSlice; ++slice) { // slice
		float theta = slice * degreePerSlice;
		v.pos = glm::vec3(radius1 * cos(theta), -height * 0.5f, radius1 * sin(theta));
		v.normal = glm::vec3(cos(theta), 0, sin(theta));
		vertex_buffer_data.push_back(v);

		v.pos = glm::vec3(radius2 * cos(theta), height * 0.5f, radius2 * sin(theta));
		v.normal = glm::vec3(cos(theta), 0, sin(theta));
		vertex_buffer_data.push_back(v);
	...
	{
		float theta = slice * degreePerSlice;
		v.pos = glm::vec3(radius2 * cos(theta), 0.5f * height, radius2 * sin(theta));
		v.normal = glm::vec3(0, 1, 0);
		vertex_buffer_data.push_back(v);
	}
	// Create the new mesh
