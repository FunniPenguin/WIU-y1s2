...
void MusicBuffer::Play()
{
	...
	alSourceQueueBuffers(this->p_Source, i, p_Buffers);
	alSourcePlay(this->p_Source);
	if (alGetError() != AL_NO_ERROR)
	{
		std::cout << "Error starting playback";
	}
...
void MusicBuffer::UpdateBufferStream()
{
	...
	alGetSourcei(this->p_Source, AL_SOURCE_STATE, &state);
	alGetSourcei(this->p_Source, AL_BUFFERS_PROCESSED, &processed);
	if (alGetError() != AL_NO_ERROR)
	{
	...
	while (processed > 0)
	{
		...
		alSourceUnqueueBuffers(this->p_Source, 1, &bufid);
		processed--;
		...
		if (slen > 0)
		{
			...
			alBufferData(bufid, p_Format, p_Membuf, (ALsizei)slen,
				p_Sfinfo.samplerate);
			alSourceQueueBuffers(this->p_Source, 1, &bufid);
		}
		if (alGetError() != AL_NO_ERROR)
		{
	...
	if (state != AL_PLAYING && state != AL_PAUSED)
	{
		...
		alGetSourcei(this->p_Source, AL_BUFFERS_QUEUED, &queued);
		if (queued == 0)
		{
			return;
		}
...
MusicBuffer::MusicBuffer(const char* filename)
{
	...
	if (!p_SndFile)
	{
		std::cout << "could not open provided music file -- check path\n";
	}
	...
	if (!p_Format)
	{
		sf_close(p_SndFile);
		p_SndFile = NULL;
		std::cout << "Unsupported channel count from file";
	}
